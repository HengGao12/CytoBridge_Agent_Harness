...
Mathematical note on the default growth semantics:

- `growth_rate` represents a rate field `g(t, x) = d/dt log w`, not an
  absolute mass value
- in the default flow-matching UOT workflow, `MassStrategy` defines the
  terminal per-particle mass target for one adjacent interval
- the conditional path defines how local log-mass evolves to reach that target
- under exact fit, velocity and growth jointly recover the intended late
  marginal interval by interval
