...
class ConditionalFlowMatcher:
    ...
    def sample_time(self, x0: torch.Tensor, x1: torch.Tensor) -> torch.Tensor:
        """
        Sample local time coordinates for a batch.

        Developers can override this to implement non-uniform time curricula
        without rewriting the rest of the path construction logic.
        """
        del x1
        return torch.rand(x0.shape[0], device=x0.device, dtype=x0.dtype)
    ...
    def compute_conditional_mass(
        self,
        terminal_mass: torch.Tensor,
        t: Union[float, torch.Tensor],
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Convert a terminal per-particle mass target into:

        1. a growth-rate regression target
        2. per-sample training loss weights

        Semantics:
        - `terminal_mass` means: if a particle has unit mass at the source side
          of the local interval (`t=0`), what mass should it have at `t=1`?
        - the conditional path, not `MassStrategy`, defines how that mass target
          is traversed over local time and how per-sample loss weights are
          derived from it.

        Default behavior keeps the current package semantics:
        - log-mass evolves linearly in local time
        - the returned `gt` is the local target for `d/dt_local log w`
        - the returned loss weights follow the existing RUOT-style reweighting
          used by the training losses
        - those loss weights are not the current path mass `w(t)`
        """
        terminal_mass = torch.as_tensor(terminal_mass)
        if terminal_mass.dim() == 1:
            terminal_mass = terminal_mass.unsqueeze(1)

        t = torch.as_tensor(
            t,
            device=terminal_mass.device,
            dtype=terminal_mass.dtype,
        ).reshape(-1, 1)

        eps = 1e-10
        source_mass = torch.ones_like(terminal_mass, device=terminal_mass.device)
        gt = torch.log(terminal_mass + eps) - torch.log(source_mass + eps)
        loss_weights = torch.exp((t - 1.0) * gt)
...
class ConditionalRegularizedUnbalancedFlowMatcher(ConditionalFlowMatcher):
    ...
    def sample_location_and_conditional_flow(self, x0: torch.Tensor, x1: torch.Tensor, 
        ...
        Computes additional terms:
        - `gt_samp`: growth-rate regression target
        - `loss_weights`: per-sample training loss weights
        """
        ...
        # Compute UOT-aware terms:
        # - `gt_samp`: growth-rate target
        # - `loss_weights`: training loss weights, not current path mass
        gt_samp, loss_weights = self.compute_cond_g(uot_plan, idx_0, idx_1, t)
        
        if return_noise:
            return t, xt, ut, eps, gt_samp, loss_weights
        else:
            return t, xt, ut, gt_samp, loss_weights
    ...
    def compute_cond_g(self, uot_plan: np.ndarray, idx_0: np.ndarray, idx_1: np.ndarray, 
        ...
        Returns:
            Tuple of (gt_samp, loss_weights) – both shape [batch_size, 1].
        """
        # Extract UOT plan rows corresponding to the current batch
        selected_uot_plan = uot_plan[idx_0]
        
        # Compute source-side terminal mass targets (sum over target dimensions)
        ...
        # Compute the legacy RUOT-style training reweighting.
        # This is not the current path mass `w(t)`.
        loss_weights = (source_weights + eps) ** (t.unsqueeze(1) - 1)
...
class WFRTravelingGaussianPath(ConditionalFlowMatcher):
    """Traveling-Gaussian WFR conditional path used by WFR-FM."""
    ...
    def compute_sigma_t(self, t: torch.Tensor) -> torch.Tensor:
        return torch.full_like(t, float(self.sigma))
    ...
    def _pair_parameters(
        ...
        eps = self.epsilon
        tm = torch.clamp(terminal_mass, min=eps)
        displacement = x1 - x0
        dist = torch.linalg.norm(displacement, dim=1, keepdim=True)
        angle = torch.clamp(dist / (2.0 * self.delta), max=(math.pi / 2.0) - self.angle_margin)
        tau = torch.tan(angle)
        sqrt_term = torch.sqrt(torch.clamp(tm / (1.0 + tau.pow(2)), min=eps))
        A = 1.0 + tm - 2.0 * sqrt_term
        B = 1.0 - sqrt_term
        denom_sq = torch.clamp(A - B.pow(2), min=eps)
        denom = torch.sqrt(denom_sq)
        direction = displacement / torch.clamp(dist, min=eps)
        omega_scalar = 2.0 * self.delta * tau * sqrt_term
        omega = omega_scalar * direction
        zero_mask = dist <= eps
        if zero_mask.any():
            omega = torch.where(zero_mask.expand_as(omega), torch.zeros_like(omega), omega)
        return {"A": A, "B": B, "denom": denom, "omega": omega}

    def _mass_and_integral(
        self,
        ...
        B = params["B"]
        denom = params["denom"]
        m_t = torch.clamp(A * t_col.pow(2) - 2.0 * B * t_col + 1.0, min=self.epsilon)
        integral = (torch.atan((A * t_col - B) / denom) - torch.atan((-B) / denom)) / denom
        return m_t, integral
    ...
    def sample_xt_wfr(
        ...
        params = self._pair_parameters(x0, x1, terminal_mass)
        _, integral = self._mass_and_integral(params, t)
        eta_t = x0 + params["omega"] * integral
        sigma_t = torch.as_tensor(self.sigma, dtype=x0.dtype, device=x0.device)
        return eta_t + sigma_t * epsilon

    def compute_conditional_flow_wfr(
        self,
        x0: torch.Tensor,
        x1: torch.Tensor,
        terminal_mass: torch.Tensor,
        ...
        del xt
        params = self._pair_parameters(x0, x1, terminal_mass)
        m_t, _ = self._mass_and_integral(params, t)
        return params["omega"] / m_t

    def compute_conditional_mass_wfr(
        self,
        x0: torch.Tensor,
        x1: torch.Tensor,
        terminal_mass: torch.Tensor,
...
def get_batch_uot_fm(FM: ConditionalRegularizedUnbalancedFlowMatcher, X: list[np.ndarray], 
    ...
    Returns:
        Tuple of concatenated tensors across all time steps:
            ...
            loss_weights_t: UOT-aware training loss weights (shape [total_samples, 1]).
    ...
    for time_idx in range(t_train.size(0) - 1):
        ...
        # Sample flow matching terms (includes UOT-aware gt and training loss weights)
        t_sampled, xt, ut, eps, gt_samp, loss_weights = FM.sample_location_and_conditional_flow(
            x0=sampled_x0,
            x1=sampled_x1,
        ...
        loss_weight_ts.append(loss_weights)
    ...
    concat_loss_weights = torch.cat(loss_weight_ts) if loss_weight_ts else torch.empty(0, 1, device=device)
    concat_noises = torch.cat(noises) if noises else torch.empty(0, *X[0].shape[1:], device=device)

    return concat_t, concat_xt, concat_ut, concat_gt, concat_loss_weights, concat_noises
